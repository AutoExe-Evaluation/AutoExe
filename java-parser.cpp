...
static bool isType(const TSNode& node)
{
    return node == "void_type" || node == "integral_type" || node == "floating_point_type" || node == "boolean_type"
        || node == "generic_type" || node == "type_identifier";
}
    ...
    std::vector<NodePtr> &Ps)
{
    ...
    else if (node == "method_declaration")
    {
    ...
    else if (node == "attributed_declarator")
        return parseFuncDecl(file, get(node, (unsigned)0), Ps);
    else
        return parseFuncDecl(file, get(node, "name"), Ps);
}
                      ...
                      std::vector<NodePtr> &calls, NodePtr &detail, bool is_lval = false)
{
    ...
    else if (node == "field_access")
    {
        ...
        NodePtr attrDetail = getDetail(file, get(node, "field"));
        detail = std::make_unique<Attribute>(
            file, lo(node), hi(node), std::move(objDetail), ".", std::move(attrDetail));
    }
    else if (node == "decimal_integer_literal" || node == "hex_integer_literal" || node == "octal_integer_literal" || node == "binary_integer_literal"
          || node == "decimal_floating_point_literal" || node == "hex_floating_point_literal")
    ...
    else if (node == "null_literal")
        detail = std::make_unique<NullLiteral>(file, lo(node), hi(node));
    else if (node == "character_literal")
        detail = std::make_unique<CharLiteral>(file, lo(node), hi(node));
    ...
    else if (node == "method_invocation" || node == "object_creation_expression")
    {
        TSNode func = node == "method_invocation" ? get(node, "name") : get(node, "type");
        bool is_member = false;
        std::vector<NodePtr> argv;
        if (get(node, "object") != nullptr)
        {
            auto name = Chunk(file, lo(node), hi(get(node, size(node) - 2))).str();
            TSNode object = get(node, "object");
    ...
    else if (node == "array_creation_expression")
    {
        TSNode func = get(node, "type");
        Chunk f(file, lo(func), hi(func), NAME);
        std::vector<NodePtr> argv;
        for (auto child : node)
        {
            if (child == "new" || isType(child)) continue;
            if (child == "dimensions_expr")
            {
                argv.push_back(parseExpr(file, get(child, size(child) - 2)));
                continue;
            }
            if (child == "array_initializer")
            {
                for (auto inner : child)
                {
                    if (inner == "{" || inner == "}" || inner == ",") continue;
                    argv.push_back(parseExpr(file, inner));
                }
            }
        }
        detail = std::make_unique<Call>(file, lo(node), hi(node), f, argv);
    }
    else if (node == "assignment_expression")
    {
    ...
    else if (node == "update_expression")
    {
        TSNode first = get(node, 0u), arg;
        auto first_op = strip_op(str(first));
        ...
        else
        {
            // postfix
            arg = get(node, 0u);
            first_op = strip_op(str(get(node, 1u)));
        }
        parseLVal(file, arg, lvals);
        NodePtr argDetail = getDetail(file, arg, true);
        detail = std::make_unique<Unary>(file, lo(node), hi(node), std::move(argDetail), first_op);
    }
    else if (node == "unary_expression")
    {
        TSNode arg = get(node, "operand");
        NodePtr argDetail = getDetail(file, arg);
        auto op = strip_op(str(get(node, "operator")));
        detail = std::make_unique<Unary>(file, lo(node), hi(node), std::move(argDetail), op);
    }
    else if (node == "cast_expression")
    {
        NodePtr argDetail = getDetail(file, get(node, "value"));
        auto op = "(" + strip_op(str(get(node, "type"))) + ")";
        detail = std::make_unique<Unary>(file, lo(node), hi(node), std::move(argDetail), op);
    }
    else if (node == "binary_expression" || node == "instanceof_expression")
    {
        NodePtr leftDetail = getDetail(file, get(node, "left"));
        auto op = node == "instanceof_expression" ? "instanceof" : strip_op(str(get(node, "operator")));
        NodePtr rightDetail = getDetail(file, get(node, "right"));
        detail = std::make_unique<Binary>(
            file, lo(node), hi(node), std::move(leftDetail), op, std::move(rightDetail));
    }
    else if (node == "ternary_expression")
    {
        NodePtr cond = getDetail(file, get(node, "condition"));
        NodePtr thenDetail = getDetail(file, get(node, "consequence"));
        NodePtr elseDetail = getDetail(file, get(node, "alternative"));
        detail = std::make_unique<Ternary>(
            file, lo(node), hi(node), std::move(cond), std::move(thenDetail), std::move(elseDetail));
    }
    else if (node == "array_access")
    {
        NodePtr argDetail = getDetail(file, get(node, "array"));
        NodePtr indexDetail = getDetail(file, get(node, "index"));
...
//    info("parseDecl(%s%s%s)\n", YELLOW, str(node).c_str(), OFF);
    ...
    else if (node == "variable_declarator")
    {
        Chunk x = parseIdentifier(file, get(node, 0u));
        if (x.empty())
            return;
        NodePtr e;
        if (auto value = get(node, "value"); value != nullptr)
            e = parseExpr(file, value);
        else e.reset(new Empty(file, hi(node)));
        decl.insertVar(x, e);
    }
    else
    {
...
static NodePtr parseForInit(const File &file, TSNode node)
{
    if (node == nullptr)
        return NodePtr(nullptr);
...
static NodePtr parseAST(const File &file, TSNode node)
{
    if (node == nullptr)
        return NodePtr(nullptr);
    else if (node == "line_comment" || node == "block_comment")
    ...
    else if (node == "if_statement")
    {
        ...
        NodePtr _then = parseAST(file, get(node, "consequence"));
        NodePtr _else = parseAST(file, get(node, "alternative"));
        if (_cond && _then)
        {
            std::vector<NodePtr> child;
            child.push_back(MOVE(_then));
            if (_else) child.push_back(MOVE(_else));
            else child.emplace_back(new Empty(file, hi(node)));
            return NodePtr(new IfThenElse(file, lo(node), hi(node), _cond, child));
        }
    }
    else if (node == "else_clause")
    {
        auto i = after(node, "else");
        if (i != end(node))
            return parseAST(file, *i);
    }
    else if (node == "switch_statement")
    {
        NodePtr _cond = parseExpr(file, get(node, "condition"));
        NodePtr _body = parseAST(file, get(node, "body"));
        if (_cond && _body)
    ...
    else if (node == "for_statement" || node == "enhanced_for_statement")
    {
        NodePtr _init;
        if (node == "for_statement")
            _init = parseForInit(file, get(node, "init"));
        else
        {
            for (auto child : node)
                if (child == "identifier")
                {
                    _init = parseForInit(file, child, get(node, "value"));
                    break;
                }
        }
        NodePtr _cond = parseExpr(file, node == "enhanced_for_statement" ? get(node, "value") : get(node, "condition"));
        NodePtr _incr = node == "enhanced_for_statement" ? nullptr : parseExpr(file, get(node, "update"));
        NodePtr _body = parseAST(file, get(node, "body"));
        if (_body)
        {
    ...
    else if (node == "case_statement")
    {
        ...
        {
            auto child = *i;
            if (child == "default") { ++i; continue; }
            if (child == "case") { ++i; ++i; continue; }
            NodePtr x = parseAST(file, child);
            if (x)
                elems.push_back(std::move(x));
        }
...
static void parseFunc(const File &file, TSNode func)
{
    std::vector<NodePtr> Ps;
    Chunk name = parseFuncDecl(file, func, Ps);
    NodePtr body = parseAST(file, get(func, "body"));
    if (body == nullptr || body->str() == ";")
    ...
    if (name.kind != END)
    {
        ...
        NodePtr f(new Func(file, lo(func), hi(func), name, Ps, body));
    ...
    std::vector<NodePtr> &Fs)
{
    ...
    for (auto child: node)
    {
        if (child == "field_declaration")
        {
            NodePtr node(new Decl(file, lo(child), hi(child)));
            Decl &decl = dynamic_cast<Decl &>(*node);
            parseField(file, child, decl);
            if (decl.size() > 0)
                Fs.push_back(std::move(node));
        }
        else if (child == "line_comment" || child == "block_comment")
        {
            NodePtr c(new Comment(file, lo(child), hi(child)));
            Fs.push_back(std::move(c));
        }
        else if (child == "method_declaration")
            parseFunc(file, child);
    }
}

static Struct *parseClass(const File &file, TSNode _struct)
{
...
static void parseTypedefNames(const File &file, TSNode node, std::vector<Chunk> &names)
{
    ...
    else if (node == "array_declarator" || node == "method_declaration")
...
static void parseTypedef(const File &file, TSNode _typedef)
{
    ...
    if (child == nullptr)
        return;
    else if ((child == "struct_specifier" || child == "union_specifier") &&
    ...
    std::vector<Chunk> &includes)
{
    ...
    for (auto child: root)
    {
        ...
        if (child == "method_declaration")
            parseFunc(file, child);
        else if (child == "class_declaration")
        {
            prev = parseClass(file, child);
            continue;
        }
        ...
        else if (child == "local_variable_declaration")
        {
