    ...
    std::vector<NodePtr> &Ps)
{
    ...
    {
        Chunk f = parseFuncDecl(file, get(node, "declarator"), Ps);
        TSNode params = get(node, "parameters");
        if (params == nullptr)
            return f;
        for (auto child: params)
        {
            if (child == "parameter_declaration")
            {
    ...
    std::vector<NodePtr> &calls, NodePtr &detail, bool is_lval = false)
{
    ...
    else if (node == "string_literal" || node == "concatenated_string")
    ...
    else if (node == "call_expression" || node == "sizeof_expression")
    {
        TSNode func = node == "sizeof_expression" ? get(node, 0u) : get(node, "function");
        Chunk f(file, lo(func), hi(func), NAME);
        parseExpr(file, func, names, types, fields, lvals, calls, detail);
        TSNode args;
        if (node == "sizeof_expression")
        {
            args = get(node, "value");
            if (args == nullptr)
                args = get(node, "type");
        }
        else args = get(node, "arguments");
        ...
        NodePtr call = nullptr;
        if (f.str() == "assert")
            call = NodePtr{new Assert(file, lo(node), hi(node), argv)};
        else if (node == "sizeof_expression")
            call = NodePtr{new Sizeof(file, lo(node), hi(node), argv)};
        else call = NodePtr{new Call(file, lo(node), hi(node), f, argv)};
...
//        file.lineno(call->lo), CYAN, call->str().c_str(), OFF);
        ...
        if (f.str() == "assert")
            detail = std::make_unique<Assert>(file, lo(node), hi(node), argv);
        else if (node == "sizeof_expression")
            detail = std::make_unique<Sizeof>(file, lo(node), hi(node), argv);
        else detail = std::make_unique<Call>(file, lo(node), hi(node), *detail, argv);
        return;
    }
    ...
    else if (node == "update_expression" || node == "unary_expression")
    {
    ...
    else if (node == "binary_expression")
    {
    ...
    {
        NodePtr argDetail = getDetail(file, get(node, "argument"));
...
//        info("\n");
    ...
    {
        NodePtr _init = parseForInit(file, get(node, "initializer"));
        NodePtr _cond = parseExpr(file, get(node, "condition"));
        NodePtr _incr = parseExpr(file, get(node, "update"));
        NodePtr _body = parseAST(file, get(node, "body"));
        if (_body)
        {
...
{
    std::vector<NodePtr> Ps;
    Chunk name = parseFuncDecl(file, get(func, "declarator"), Ps);
...
static Struct *parseStruct(const File &file, TSNode _struct)
{
    ...
    if (child == nullptr || child != "type_identifier")
        return nullptr;
    ...
    std::vector<Chunk> &includes)
{
    ...
    for (auto child: root)
    {
        ...
        else if (child == "struct_specifier")
        {
